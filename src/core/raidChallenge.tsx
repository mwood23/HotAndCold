...
import { RaidWordList } from './raidWordList.js';
import { RaidChallengeToWord } from './raidChallengeToWord.js';
import { RaidChallengeToPost } from './raidChallengeToPost.js';

export * as RaidChallenge from './raidChallenge.js';

export const getCurrentChallengeNumberKey = () => 'raid:current_challenge_number' as const;

export const getChallengeKey = (challenge: number) => `raid:challenge:${challenge}` as const;
...
export const makeNewChallenge = zoddy(
  ...
  async ({ context }) => {
    ...
    const [wordList, usedWords, currentChallengeNumber, currentSubreddit] = await Promise.all([
      RaidWordList.getCurrentWordList({
        redis: context.redis,
      }),
      RaidChallengeToWord.getAllUsedWords({
        redis: context.redis,
      }),
    ...
    try {
      ...
      await RaidWordList.setCurrentWordListWords({
        redis: txn,
      ...
      await RaidChallengeToWord.setChallengeNumberForWord({
        challenge: newChallengeNumber,
        redis: txn,
        word: newWord,
      });
      await RaidChallengeToPost.setChallengeNumberForPost({
        challenge: newChallengeNumber,
        postId: post.id,
        redis: txn,
      });
