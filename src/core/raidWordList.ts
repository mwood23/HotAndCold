...
export * as RaidWordList from './wordList.js';
...
export const getWordListKey = (dictionary = 'default') => `raid:word_list:${dictionary}` as const;
...
export const initialize = zoddy(
  ...
  async ({ context }) => {
    ...
    if (!wordList) {
      await context.redis.set(getWordListKey(), JSON.stringify([]));
    } else {
